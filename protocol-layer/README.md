...
```bash
# 本地 dev（含 Redis + Kafka + MySQL + Prometheus + Grafana）
...
│   │   └── event-bridge.ts       # Baileys → Kafka 事件桥接
│   │
...
npm test           # 单元测试
npm run test:e2e   # e2e（需 redis + kafka）
```
