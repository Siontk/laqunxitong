...
REDIS_URL=redis://:REPLACE_WITH_STRONG_PASSWORD@redis-server-private-ip:6379
REGISTRY_REDIS_URL=redis://:REPLACE_WITH_STRONG_PASSWORD@redis-server-private-ip:6379
KEYS_REDIS_URL=redis://:REPLACE_WITH_STRONG_PASSWORD@redis-server-private-ip:6379
RATELIMIT_REDIS_URL=redis://:REPLACE_WITH_STRONG_PASSWORD@redis-server-private-ip:6379
RUNTIME_REDIS_URL=redis://:REPLACE_WITH_STRONG_PASSWORD@redis-server-private-ip:6379
REDIS_KEY_PREFIX=unsea:
...
RECONNECT_BURST=20
WORKER_GROUP_OP_PER_SEC=50
WORKER_GROUP_OP_BURST=100
GROUP_ACCOUNT_LOCK_TTL_MS=90000
GROUP_ACCOUNT_BUSY_RETRY_MS=3000
WORKER_GROUP_BUSY_RETRY_MS=2000
NODE_ONLINE_PER_SEC=25
NODE_ONLINE_BURST=50
GLOBAL_ONLINE_PER_SEC=50
ACCOUNT_ONLINE_COOLDOWN_MS=5000
BATCH_ONLINE_MAX_SIZE=200
...
`PUBLIC_ENDPOINT` 必须写功能层能访问的内网地址或域名，不要写 `0.0.0.0`。`HTTP_HOST=0.0.0.0` 只表示协议进程监听所有网卡。

三台服务器联调建议先用 `WORKER_ROLE=standalone` 单协议进程跑通链路；如果要模拟单机多 worker，用 PM2 配置启动 `8081-8084` 四个 worker，并给每个 worker 注册不同 `PUBLIC_ENDPOINT`。
...
11. 批量调用 `POST /v1/accounts/online/batch` 时，功能层能处理 `results[]` 和 `remote[]`。
12. Kafka owner topic 能消费到 `account.owner_assigned`，并正确处理 `reason=batch_online`。
